function ExportReshapedBinaryFieldResult(fileName, imOpt, imVal, PSLthickness, boundaryThickness);
	...
	passiveEles0 = [srcPSList.eleIndexList]';
	index = 2;
	while index<=PSLthickness
		passiveEles0 = RelateAdjacentElements(passiveEles0);
		index = index + 1;
    end
	passiveEles1 = passiveEles0;
	while index<=PSLthickness+2
		passiveEles1 = RelateAdjacentElements(passiveEles1);
		index = index + 1;
    end
	...
	passiveEles = unique([passiveEles0; passiveElesOnBoundary]);
	shapedDesignDomain = unique([passiveEles1; passiveElesOnBoundary]);
	numElementsNew = numel(shapedDesignDomain);
	...
	consideredEles = carEleMapBack_(shapedDesignDomain);
	consideredEles = consideredEles -1;
	consideredEles = [consideredEles ones(size(consideredEles))];
	[~,passiveElesNew] = intersect(shapedDesignDomain, passiveEles);
	consideredEles(passiveElesNew,2) = 2;
	fprintf(fid, '%d %d\n', consideredEles');
